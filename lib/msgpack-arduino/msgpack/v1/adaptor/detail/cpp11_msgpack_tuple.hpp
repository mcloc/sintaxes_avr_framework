...
inline tuple<Args...> make_tuple(Args&&... args) {
    return tuple<Args...>(boost::forward<Args>(args)...);
}
...
inline auto tuple_cat(Tuples&&... args) ->
    decltype(
    		boost::tuple::tuple_cat(boost::forward<typename boost::remove_reference<Tuples>::type::base>(args)...)
    ) {
    return boost::tuple::tuple_cat(boost::forward<typename boost::remove_reference<Tuples>::type::base>(args)...);
}
...
struct MsgpackTupleConverter {
    static void convert(
        ...
        if (o.via.array.size >= N)
            o.via.array.ptr[N-1].convert<typename boost::remove_reference<decltype(type::get<N-1>(v))>::type>(type::get<N-1>(v));
    }
};
...
struct MsgpackTupleConverter<Tuple, 1> {
    static void convert (
        msgpack::object const& o,
        Tuple& v) {
        o.via.array.ptr[0].convert<typename boost::remove_reference<decltype(type::get<0>(v))>::type>(type::get<0>(v));
    }
};
...
template <typename... Args>
struct as<msgpack::type::tuple<Args...>, typename boost::enable_if<msgpack::any_of<msgpack::has_as, Args...>::value>::type>  {
