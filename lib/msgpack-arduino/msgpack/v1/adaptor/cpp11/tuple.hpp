...
struct pack<boost:tuple<Args...>> {
    template <typename Stream>
    msgpack::packer<Stream>& operator()(
        msgpack::packer<Stream>& o,
        const boost::tuple<Args...>& v) const {
...
struct StdTupleAsImpl {
    static boost:tuple<T, Args...> as(msgpack::object const& o) {
        return boost:tuple_cat(
...
template <typename... Args>
struct StdTupleAs {
    static boost:tuple<Args...> as(msgpack::object const& o) {
...
template <>
struct StdTupleAs<> {
    static boost:tuple<> as (msgpack::object const&) {
        return boost:tuple<>();
    }
};
...
struct StdTupleConverter {
    static void convert(
        ...
        if (o.via.array.size >= N)
            o.via.array.ptr[N-1].convert<typename boost::remove_reference<decltype(boost::get<N-1>(v))>::type>(boost::get<N-1>(v));
    }
};
...
template <typename... Args>
struct as<boost:tuple<Args...>, typename std::enable_if<msgpack::any_of<msgpack::has_as, Args...>::value>::type>  {
    boost:tuple<Args...> operator()(
...
template <typename... Args>
struct convert<boost:tuple<Args...>> {
    msgpack::object const& operator()(
        msgpack::object const& o,
        boost:tuple<Args...>& v) const {
        // if(o.type != msgpack::type::ARRAY) { throw msgpack::type_error(); }
...
struct StdTupleToObjectWithZone {
    static void convert(
        ...
        o.via.array.ptr[N-1] = msgpack::object(boost::get<N-1>(v), o.zone);
    }
};
...
template <typename... Args>
struct object_with_zone<boost:tuple<Args...>> {
    void operator()(
        msgpack::object::with_zone& o,
        boost:tuple<Args...> const& v) const {
        uint32_t size = checked_get_container_size(sizeof...(Args));
